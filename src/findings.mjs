...
// Bounds on how much of an unexpected key set is quoted back. A drifted output carries a key
// or two; anything longer is the model pasting diff content into a field name, which is
// exactly the case that must not reach the prompt intact.
const MAX_REPORTED_FIELDS = 8;
const MAX_REPORTED_FIELD_CHARS = 64;

// Render untrusted key names for a message that becomes a prompt.
//
// Keys observed on the wire are model output, and the model's input is the PR diff - so a key
// name is attacker-influenced text from a repository this engine does not control. Since the
// message is echoed verbatim into the repair prompt, an unencoded key containing newlines or
// instruction text would be writing into that prompt. JSON.stringify escapes control
// characters and quotes; the count and length caps keep a pathological output from filling
// the prompt.
...
//
// `missing` and `expected` are subsets of this module's own field constants, so they are
// quoted plainly; only `unexpected` comes from the model and needs encoding.
